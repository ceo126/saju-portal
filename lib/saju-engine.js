...
class SajuEngine {
  ...
  calculate(year, month, day, hour = -1, gender = 'male') {
    ...
    // 오행 보충 가이드 (부족한 오행)
    const ohengGuide = ohengAnalysis.missing.map(oh => ({
      oheng: oh,
      ...OHENG_BALANCE_GUIDE[oh]
    }));

    // 일주 특성
    const iljuKey = dayPillar.cheongan + dayPillar.jiji;
    const iljuSpecial = ILJU_SPECIAL[iljuKey] || null;
  ...
  // 월운 간지 계산 (특정 년도 12개월)
  getMonthlyGanji(year) {
    const yearPillar = this.getYearPillarForYear(year);
    const months = [];
    for (let m = 1; m <= 12; m++) {
      // 양력 월 중순(15일)을 기준으로 사주 월 계산
      const sajuMonth = this.getSajuMonth(m, 15);
      const monthJijiIdx = (sajuMonth + 1) % 12;
      const startIdx = MONTH_CHEONGAN_START[yearPillar.cheongan];
      const ganIdx = (startIdx + (sajuMonth - 1)) % 10;
      months.push({
        month: m,
        cheongan: CHEONGAN[ganIdx],
        ...
        jijiHanja: JIJI_HANJA[monthJijiIdx],
        ganji: CHEONGAN[ganIdx] + JIJI[monthJijiIdx],
        oheng: OHENG[CHEONGAN[ganIdx]],
        jijiOheng: OHENG_JIJI[JIJI[monthJijiIdx]]
      });
    }
    return months;
  }
  ...
  getSeunAnalysis(sajuResult) {
    ...
    const monthlyGanji = this.getMonthlyGanji(2026);
