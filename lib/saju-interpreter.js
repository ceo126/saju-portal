...
반드시 유효한 JSON만 출력하세요. 마크다운 코드블록 없이 순수 JSON만 출력하세요.`;
    ...
    try {
      ...
      return this.getFallbackInterpretation(sajuResult);
    }
  }

  // 궁합 해석
  async interpretCompatibility(compatResult) {
    const { saju1, saju2, score, details } = compatResult;
...

{
  ...
  "scores": {
    "love": ${Math.min(99, score + Math.floor(Math.random() * 10))},
    "communication": ${Math.min(99, score + Math.floor(Math.random() * 15) - 5)},
    "values": ${Math.min(99, score + Math.floor(Math.random() * 10) - 3)},
    "future": ${Math.min(99, score + Math.floor(Math.random() * 12) - 2)}
  }
}
...
반드시 유효한 JSON만 출력하세요. 마크다운 코드블록 없이 순수 JSON만 출력하세요.`;
    ...
    try {
      ...
      return {
        ...
        scores: { love: score, communication: score - 3, values: score + 2, future: score + 5 }
      };
    }
  }
...
반드시 유효한 JSON만 출력하세요. 마크다운 코드블록 없이 순수 JSON만 출력하세요.`;
  ...
  // AI 실패시 기본 해석
  getFallbackInterpretation(sajuResult) {
