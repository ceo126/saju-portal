...
class SajuInterpreter {
  ...
  // AI 호출 with 타임아웃 + 재시도
  async generateWithTimeout(prompt, retries = 1) {
    let timer;
    const timeoutPromise = new Promise((_, reject) => {
      timer = setTimeout(() => reject(new Error('AI 응답 타임아웃 (30초)')), this.requestTimeout);
    });
    try {
      ...
      // 403/401 등 인증 오류는 재시도 무의미
      if (retries > 0 && !e.message.includes('403') && !e.message.includes('401') && !e.message.includes('API key')) {
        console.warn(`AI 호출 재시도 (남은 횟수: ${retries}):`, e.message);
        clearTimeout(timer);
        await new Promise(r => setTimeout(r, 1000));
        return this.generateWithTimeout(prompt, retries - 1);
      }
      throw e;
    } finally {
      clearTimeout(timer);
  ...
  extractJSON(text) {
    ...
    try {
      ...
      cleaned = cleaned
        ...
        .replace(/(?<=[\[{,:\s])'|'(?=[\]},:\s])/g, '"')  // single quotes (key/value boundaries only)
